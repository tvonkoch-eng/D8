...
async def get_date_recommendations(request: DateRequest):
    ...
    try:
        ...
        # 2. Check if we have good quality places, if not use fallback
        should_use_fallback = False
        
        if not places:
            should_use_fallback = True
            print("No OSM results, using fallback places...")
        elif len(places) < 3:
            should_use_fallback = True
            print("Insufficient OSM results, using fallback places...")
        elif request.price_range == 'luxury':
            # For luxury searches, check if we have any high-quality places
            luxury_places = [p for p in places if p.get('price_level') in ['high', 'luxury'] or 
                           any(keyword in p.get('name', '').lower() for keyword in ['fine', 'dining', 'restaurant', 'bistro', 'grill', 'steak'])]
            if not luxury_places:
                should_use_fallback = True
                print("No luxury places found in OSM, using fallback places...")
        ...
        if should_use_fallback:
            ...
            print(f"Using {len(places)} fallback places")
        
        # 3. Use OpenAI to filter and enhance recommendations
...
async def search_openstreetmap(request: DateRequest):
    ...
    try:
        ...
        # For luxury searches, be more aggressive
        if request.price_range == 'luxury' and places:
            # Check if we have any high-quality places
            luxury_places = [p for p in places if p.get('price_level') in ['high', 'luxury'] or 
                           any(keyword in p.get('name', '').lower() for keyword in ['fine', 'dining', 'restaurant', 'bistro', 'grill', 'steak'])]
            
            if not luxury_places:
                print("No luxury places found in OSM, trying broader search...")
...
async def filter_with_ai(places: List[dict], request: DateRequest):
    ...
    if not openai_api_key:
        print("No OpenAI API key available, using enhanced basic filtering")
        return await enhanced_basic_filtering(places, request)
    ...
    for attempt in range(3):  # Try up to 3 times
        try:
            print(f"OpenAI attempt {attempt + 1}/3")
            ...
            if json_match:
                ...
                if attempt == 2:  # Last attempt
                    print("OpenAI failed to return valid JSON, using enhanced basic filtering")
                    return await enhanced_basic_filtering(places, request)
                continue
            ...
            if attempt == 2:  # Last attempt
                print("OpenAI failed after 3 attempts, using enhanced basic filtering")
                return await enhanced_basic_filtering(places, request)
...
async def enhanced_basic_filtering(places: List[dict], request: DateRequest):
    """
    Enhanced basic filtering when AI is not available
    """
    if not places:
        return []
    
    print("Using enhanced basic filtering algorithm")
    ...
    for place in places:
        ...
        if request.cuisines:
            ...
            for cuisine in request.cuisines:
                if cuisine.lower() in place['cuisine'].lower() or place['cuisine'].lower() in cuisine.lower():
                    cuisine_match = True
                    break
            
            if cuisine_match:
                score += 0.4
            elif place['cuisine'] != 'various':
        ...
        if request.price_range:
            ...
            if place['price_level'] in price_match.get(request.price_range, []):
                score += 0.25
            elif request.price_range == 'luxury' and place['price_level'] == 'high':
                score += 0.15  # High is close to luxury
            elif request.price_range == 'high' and place['price_level'] == 'luxury':
                score += 0.15  # Luxury is close to high
        
        # 3. Date type appropriateness (20% weight)
        if request.date_type == 'luxury':
            if place['price_level'] in ['high', 'luxury']:
                score += 0.2
            elif place.get('amenity') == 'restaurant':
                score += 0.1
        elif request.date_type == 'casual':
            if place['price_level'] in ['low', 'medium']:
                score += 0.2
            elif place.get('amenity') in ['cafe', 'bistro']:
                score += 0.15
        elif request.date_type == 'romantic':
            if place.get('amenity') == 'restaurant' and place['price_level'] in ['medium', 'high', 'luxury']:
                score += 0.2
            elif place.get('amenity') in ['cafe', 'bistro']:
                score += 0.1
        
        # 4. Meal time appropriateness (10% weight)
        if request.meal_times:
            if 'dinner' in request.meal_times and place.get('amenity') == 'restaurant':
                score += 0.1
            elif 'breakfast' in request.meal_times and place.get('amenity') in ['cafe', 'bistro']:
                score += 0.1
            elif 'lunch' in request.meal_times:
                score += 0.05  # Most places are good for lunch
        
        # 5. Restaurant quality indicators (5% weight)
        if place.get('amenity') == 'restaurant':
            score += 0.05
        elif place.get('amenity') in ['cafe', 'bistro']:
            score += 0.03
        ...
        # Create better why_recommended text
        why_recommended = f"Perfect for {request.date_type}"
        if place['cuisine'] != 'various':
            why_recommended += f" with {place['cuisine']} cuisine"
        if place.get('amenity') == 'restaurant':
            why_recommended += " in a restaurant setting"
        ...
        # Add price context
        if request.price_range and place['price_level'] == request.price_range:
            why_recommended += f" with {place['price_level']} pricing"
...
async def search_osm_places(lat: float, lon: float, request: DateRequest):
    ...
    try:
        ...
        # Build price filter for luxury searches
        price_filter = ""
        if request.price_range == 'luxury':
            price_filter = '["price_level"~"3|4"]'
        elif request.price_range == 'high':
            price_filter = '["price_level"~"2|3|4"]'
        
        # Overpass query for restaurants and cafes - prioritize restaurants
        # Search in a larger radius for luxury places
        search_radius = 2000 if request.price_range == 'luxury' else 1000
        
        query = f"""
        [out:json][timeout:25];
        (
          node["amenity"="restaurant"]{cuisine_filter}{price_filter}(around:{search_radius},{lat},{lon});
          way["amenity"="restaurant"]{cuisine_filter}{price_filter}(around:{search_radius},{lat},{lon});
          relation["amenity"="restaurant"]{cuisine_filter}{price_filter}(around:{search_radius},{lat},{lon});
          node["amenity"~"cafe|bar|bistro"]{cuisine_filter}(around:{search_radius},{lat},{lon});
          way["amenity"~"cafe|bar|bistro"]{cuisine_filter}(around:{search_radius},{lat},{lon});
          relation["amenity"~"cafe|bar|bistro"]{cuisine_filter}(around:{search_radius},{lat},{lon});
        );
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(url, data=data, headers=headers) as response:
                if response.status == 200:
                    ...
                    places = convert_osm_to_places(result.get("elements", []))
                    
                    # If no results and looking for luxury, try without price filter
                    if not places and request.price_range == 'luxury':
                        print("No luxury places found, trying broader search...")
                        return await search_osm_places_broad(lat, lon, request)
...
def generate_fallback_places(lat: float, lon: float, request: DateRequest):
    ...
    base_places = [
        ...
        },
        {
            ...
            "description": "Luxury fine dining with exceptional service",
            "cuisine": "french",
            "price_level": "luxury",
            "rating": 4.8,
            "amenities": ["Fine Dining", "Wine Cellar", "Valet Parking", "Private Dining"]
        },
        {
            "name": "Steakhouse Prime",
            "description": "Premium steakhouse with aged cuts and wine selection",
            "cuisine": "steakhouse",
            "price_level": "luxury",
            "rating": 4.7,
            "amenities": ["Premium Cuts", "Wine Selection", "Private Booths"]
        },
        {
            "name": "Sushi Master",
            ...
            "price_level": "high",
            "rating": 4.5,
            "amenities": ["Fresh Sushi", "Sake Bar", "Chef's Counter"]
        },
        {
            "name": "Mediterranean Breeze",
            "description": "Elegant Mediterranean restaurant with ocean views",
            "cuisine": "mediterranean",
            "price_level": "high",
            "rating": 4.4,
            "amenities": ["Ocean Views", "Fresh Seafood", "Outdoor Terrace"]
        },
        {
            "name": "The Artisan Kitchen",
            "description": "Contemporary cuisine with artistic presentation",
            "cuisine": "contemporary",
            "price_level": "luxury",
            "rating": 4.6,
            "amenities": ["Artistic Presentation", "Tasting Menu", "Sommelier"]
        }
    ]
    
    # Filter by cuisine and price range if specified
    filtered_places = []
    for place in base_places:
        # Check cuisine match
        cuisine_match = True
        if request.cuisines:
            cuisine_match = any(cuisine.lower() in place["cuisine"].lower() or place["cuisine"].lower() in cuisine.lower() for cuisine in request.cuisines)
        
        # Check price range match
        price_match = True
        if request.price_range:
            price_match = place["price_level"] == request.price_range
            # Allow some flexibility for luxury searches
            if request.price_range == 'luxury' and place["price_level"] == 'high':
                price_match = True
            elif request.price_range == 'high' and place["price_level"] == 'luxury':
                price_match = True
    ...
    # If we have filtered results, use them; otherwise use all places
    if filtered_places:
        base_places = filtered_places
    else:
        # If no matches, prioritize by date type
        if request.date_type == 'luxury':
            base_places = [p for p in base_places if p["price_level"] in ['high', 'luxury']]
        elif request.date_type == 'casual':
            base_places = [p for p in base_places if p["price_level"] in ['low', 'medium']]
    ...
    for i, place in enumerate(base_places):
        ...
        # Create more realistic addresses based on location
        if "portland" in request.location.lower():
            street_names = ["NW 23rd Ave", "SE Hawthorne Blvd", "NE Alberta St", "SW 10th Ave", "NE Broadway", "SW Morrison St"]
            address = f"{100 + i * 50} {random.choice(street_names)}, Portland, OR"
        else:
        ...
        place_data = {
            ...
            "amenity": "restaurant" if place["price_level"] in ["high", "luxury"] else "cafe"
        }
